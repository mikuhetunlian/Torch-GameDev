	...
	public class Slicer 
	{
		...
		static private Slice2D MultipleSlice(Polygon2D polygon, Pair2D slice) 
		{
			...
			foreach (Pair2D id in slices) 
			{
				...
				List<Polygon2D> polygonList = new List<Polygon2D>(result.GetPolygons());
				foreach (Polygon2D poly in polygonList)
				{
		...
		/// 计算相交点坐标
		/// </summary>
		/// <param name="polygon">原物体的polygon</param>
		/// <param name="slice">输入的pair</param>
		/// <returns></returns>
