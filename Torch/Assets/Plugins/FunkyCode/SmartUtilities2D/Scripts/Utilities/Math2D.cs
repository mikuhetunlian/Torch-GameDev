...
namespace Utilities2D {
	public class Math2D 
	{
		...
		public static Vector2D GetPointLineIntersectLine(Pair2D lineA, Pair2D lineB)
		{
			...
			double dx_cx = lineB.B.x - lineB.A.x;//B deltaX
			double dy_cy = lineB.B.y - lineB.A.y;//B deltaY

			double bx_ax = lineA.B.x - lineA.A.x;//A deltaX
			double by_ay = lineA.B.y - lineA.A.y;//A deltaY
